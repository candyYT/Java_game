...
public class Player extends Entity {
    ...
    public void update() {
        ...
        // Урон от обычных монстров (боссы исключены)
        if (!invincible) {
            for (int i = 0; i < gp.monster.length; i++) {
                if (gp.monster[i] != null) {
                    // Пропускаем боссов – они наносят урон только атаками
                    if (gp.monster[i] instanceof boss.Boss) continue;
        ...
        // ===== КОЛЛИЗИЯ С БОССОМ (улучшенная) =====
        if (gp.currentBoss != null && gp.currentBoss.life > 0 && !dash.isDashing) {
            Rectangle playerRect = new Rectangle(worldX + solidArea.x, worldY + solidArea.y,
                    solidArea.width, solidArea.height);
            Rectangle bossRect = new Rectangle(gp.currentBoss.worldX + gp.currentBoss.solidArea.x,
                    gp.currentBoss.worldY + gp.currentBoss.solidArea.y,
                    gp.currentBoss.solidArea.width, gp.currentBoss.solidArea.height);

            if (playerRect.intersects(bossRect)) {
                // Сначала пробуем вернуться на предыдущую позицию
                worldX = oldWorldX;
                worldY = oldWorldY;

                // Пересчитываем прямоугольники после возврата
                playerRect = new Rectangle(worldX + solidArea.x, worldY + solidArea.y,
                        solidArea.width, solidArea.height);
                bossRect = new Rectangle(gp.currentBoss.worldX + gp.currentBoss.solidArea.x,
                        gp.currentBoss.worldY + gp.currentBoss.solidArea.y,
                        gp.currentBoss.solidArea.width, gp.currentBoss.solidArea.height);

                // Если всё ещё пересекаемся (из-за движения босса), выталкиваем в сторону
                if (playerRect.intersects(bossRect)) {
                    int bossLeft = gp.currentBoss.worldX + gp.currentBoss.solidArea.x;
                    int bossRight = bossLeft + gp.currentBoss.solidArea.width;
                    int playerLeft = worldX + solidArea.x;
                    int playerRight = playerLeft + solidArea.width;

                    if (playerLeft < bossLeft) {
                        // Игрок слева от босса – выталкиваем влево
                        worldX = bossLeft - solidArea.width - solidArea.x - 1;
                    } else {
                        // Игрок справа от босса – выталкиваем вправо
                        worldX = bossRight - solidArea.x + 1;
                    }
                }
            }
        }
        ...
        // Атака игрока (левая кнопка мыши)
        ...
        // Проверка переходов и чекпоинтов
