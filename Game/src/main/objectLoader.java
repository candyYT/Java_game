...
public class objectLoader {
    ...
    public static void loadMonsters(gamepanel gp, String mapName) {
        ...
        gp.currentBoss = null;
        for (int i = 0; i < gp.monster.length; i++) {
            if (gp.monster[i] instanceof boss.Boss) {
                gp.currentBoss = (boss.Boss) gp.monster[i];
                break;
            }
        }

        // Устанавливаем флаг боя: true, если босс есть и жив
        gp.bossFightActive = (gp.currentBoss != null && gp.currentBoss.life > 0);
    }
    ...
    private static void loadFromFile(gamepanel gp, String mapName, String prefix, boolean loadObjects, boolean loadMonsters) {
        ...
        try (BufferedReader br = new BufferedReader(new InputStreamReader(is))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String id = type + "_" + col + "_" + row;

                if (loadObjects) {
                    if (gp.collectedObjects.containsKey(mapName) && gp.collectedObjects.get(mapName).contains(id)) {
                        continue;
                    }
                    switch (type) {
                        case "Gear":
                            ...
                            break;
                        case "Bolt":
                            if (!gp.player.hasBolt && objIndex < gp.obj.length) {
                ...
                if (loadMonsters) {
                    if (gp.killedMonsters.containsKey(mapName) && gp.killedMonsters.get(mapName).contains(id)) {
                        continue;
                    }
                    switch (type) {
                        case "CorruptedOne":
                            ...
                            break;
                        case "Claw": // новый тип для босса
                            if (monsterIndex < gp.monster.length) {
                                boss.MON_Claw boss = new boss.MON_Claw(gp);
                                boss.worldX = worldX;
                                boss.worldY = worldY;
                                gp.monster[monsterIndex] = boss;
                                monsterIndex++;
                            }
                    }
                }
